...
def setup_arch_queue(loop, mqtt, all_builders, arch):
    ...
    watcher = loop.create_task(_watch_queue(mqtt, queue, all_builders, arch))
    watcher.add_done_callback(functools.partial(_stop_queue, queue))

    return (queue, watcher)

async def _watch_queue(mqtt, queue, builders, arch):
    while True:
